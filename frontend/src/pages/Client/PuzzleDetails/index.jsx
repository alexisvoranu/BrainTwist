...
const PuzzleDetails = () => {
  ...
  useEffect(() => {
    sessionStorage.setItem("cameFromPuzzleDetails", "true");
  }, []);
  ...
  useEffect(() => {
    if (!puzzle || !puzzle.productId) return;
    if (userDetails) {
      fetch(
        ...
        .catch((error) =>
          console.error("Eroare la preluarea stării favoritelor:", error)
        );

      fetch(
        `${SERVER_URL}/feedbacks/checkFeedbackExists/${userDetails?.id}/${
          puzzleDetails?.productId
        }/${puzzleDetails?.puzzleType}/${puzzleDetails?.collection}`
      )
        .then((res) => res.json())
        .then((data) => {
      ...
      setIsFavorite(
        favorites.some((fav) => fav.productId === puzzle.productId)
      );
    }
  }, [userDetails, puzzle]);

  const saveProductLocallyIfNoUser = (puzzle, setIsFavorite) => {
  ...
  const handleAddFeedback = () => {
    if (!userDetails) {
      toast.warn("Trebuie să fii autentificat pentru a acorda o recenzie!", {
        position: "top-center",
        autoClose: 3000,
      ...
      navigate("/client/createFeedback", {
        state: {
          puzzleDetails,
        },
      });
  ...
  const handleFavoriteToggle = () => {
    if (!userDetails) {
      saveProductLocallyIfNoUser(puzzleDetails, setIsFavorite);
    } else {
      if (isFavorite) {
  ...
  const handleAddToCart = () => {
    const product = {
      ...
      puzzleType: puzzleDetails.puzzleType,
      subcollection: puzzleDetails.collection,
      stock: puzzleDetails.stock,
      quantity: 1,
    };

    if (userDetails) {
      fetch(`${SERVER_URL}/carts/addProductToCart`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ ...product, clientId: userDetails.id }),
      })
        .then((res) => {
          if (res.ok) {
            console.log("Produs adăugat în coșul online.");
          }
        })
        .catch((error) => {
          console.error("Error: ", error);
      ...
      const existingProductIndex = cart.findIndex(
        (item) => item.puzzleId === product.puzzleId
      );
      ...
      console.log("Produs salvat în localStorage.");
    }
  ...
  const getVisibleImages = (puzzle) => {
    if (puzzle.puzzleType === "Jigsaw") return [];
          ...
          >
            <div className="detalii-puzzle-content content2">
              ...
              >
                ...
                Prețul afișat include toate taxele și TVA-ul aplicabil.
              </div>
            ...
            <div className="price">
              ...
              <div className="buttondiv">
                ...
                {puzzle.stock > 0 ? (
                  ...
                  <h6 className="text-danger mb-4">Produs indipsonibil</h6>
                )}
                {!feedbackExists && (
                  <button
